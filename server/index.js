...
// ── Trim token (removes Windows CRLF \r from .env) ───────────────────────
const TMDB_TOKEN = (process.env.TMDB_TOKEN || "").trim();

// In production, frontend and backend share the same origin — no CORS needed.
// In dev, Vite dev server runs on a different port.
if (!IS_PROD) {
  app.use(cors({ origin: "http://localhost:5173" }));
} else {
  app.use(cors()); // allow all in production (same-origin anyway)
}
...
// ── Concurrency semaphore (max 3 parallel TMDB requests) ──────────────────
...
// ── Core TMDB fetcher (3 retries + back-off + semaphore) ─────────────────
...
app.listen(PORT, () => {
  ...
  if (!TMDB_TOKEN) {
    console.warn("⚠️   TMDB_TOKEN is not set!");
  } else {
